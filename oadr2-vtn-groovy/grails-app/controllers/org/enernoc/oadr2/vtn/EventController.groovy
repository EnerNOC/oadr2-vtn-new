	 ...
	 @SuppressWarnings("unchecked")
	  ...
	  def newEvent() {
		...
		if (program != null) {
	  	...
	  	chain(action:"events", model:[program: event.programName, error: errorMessage])
	  ...
	  def cancelEvent(){
		  ...
		  event.eiEvent.getEventDescriptor().setModificationNumber(event.eiEvent.getEventDescriptor().getModificationNumber() + 1);
		  if(event.eiEvent.getEventDescriptor().getEventStatus() != EventStatusEnumeratedType.CANCELLED)
			  event.eiEvent.getEventDescriptor().setEventStatus(EventStatusEnumeratedType.CANCELLED);
		  else
			  event.eiEvent.getEventDescriptor().setEventStatus(updateStatus(event, event.eiEvent.getEiEventSignals().getEiEventSignals().size()));
		  for(EiEventSignal e : event.eiEvent.getEiEventSignals().getEiEventSignals()){
			  e.setCurrentValue(new CurrentValue().withPayloadFloat(new PayloadFloat(updateSignalPayload(event))));
		  }
		  redirect(action: "events");
	  }
	  ...
	  def updateEvent(){
		  log.error "The Current ID is: " + params.id
		  def event = Event.get(params.id)
		  //Event newEventForm = eventForm.get();
		  //EiEvent event = JPA.em().find(EiEvent.class, id);
		  Date currentDate = new Date();
		  ...
		  for(int i=0; i < event.intervals; i++){
			  intervalList.add(new Interval()
				  .withDuration(new DurationPropType()
					  .withDuration(new DurationValue()
						  .withValue(formatDuration(getDuration(event)))))
		  ...
		  intervals.setIntervals(intervalList);
		  event.eiEvent.setEiActivePeriod(new EiActivePeriod()
				  .withProperties(new Properties()
					  .withDtstart(new Dtstart()
						  .withDateTime(new DateTime()
							  .withValue(event.eiEvent.getEiActivePeriod().getProperties().getDtstart().getDateTime().getValue().normalize())))
					  .withDuration(new DurationPropType()
						  .withDuration(new DurationValue()
							  .withValue(formatDuration(getDuration(event, (int)event.intervals)))))
					  .withTolerance(new Tolerance()
						  .withTolerate(new Tolerate()
							  .withStartafter(new DurationValue()
								  .withValue((formatDuration(getDuration("P0Y0M0DT0H0M0S")))))))
					  ...
					  .withXEiRecovery(new DurationPropType()
						  .withDuration(new DurationValue()
							  .withValue((formatDuration(getDuration("P0Y0M0DT0H0M0S"))))))));
		  event.eiEvent.setEiTarget(new EiTarget());
		  event.eiEvent.setEventDescriptor(new EventDescriptor()
					  .withCreatedDateTime(new DateTime().withValue(xCalendar))
					  .withEiMarketContext(new EiMarketContext()
							  .withMarketContext(new MarketContext()
									  .withValue(contextName)))
					  .withEventID(event.eventID)
					  .withEventStatus(updateStatus(event, (int)event.intervals))
					  .withModificationNumber(event.eiEvent.getEventDescriptor().getModificationNumber() + 1)
					  .withPriority(event.priority)
					  .withTestEvent("False")
					  .withVtnComment("No VTN Comment"));
		  event.eiEvent.setEiEventSignals(new EiEventSignals()
		  .withEiEventSignals(new EiEventSignal()
				  .withCurrentValue(new CurrentValue()
						  .withPayloadFloat(new PayloadFloat()
								  .withValue(updateSignalPayload(event)))) //TODO Not sure what this value is supposed to be, must be 0 when NEAR
